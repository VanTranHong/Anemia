...
df = pd.read_csv('/Users/vantran/Desktop/AAAAAA/resultsparallel/infogain_30.csv')
...
for i in range(row):
    rw = df.iloc[i].tolist()
    rw = [i/splits for i in rw]
    average.append(np.mean(rw))
    std.append(np.std(rw)/math.sqrt(cols))
    
new_ave =[]
new_std =[]
# Features = [1,12,13,18, 24,7,4,5, 2,3,6,8,9,10,11,14,15,16,17,19,20,21,22,23,25,26,27]
Features=[1,2,3,4,5]
for i in Features:
...
plt.tight_layout()
plt.savefig('test_based_plot_with_error_bars.png')
plt.show()
