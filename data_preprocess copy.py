...
def remove_invalid_column(data,target):
    ...
    # data2 = data[data.columns[data.isnull().mean()<0.1]]
    # data2 = data2.drop('Unnamed: 0', axis = 1)
    # if target not in data2.columns:
        
    
    #     data1 = pd.concat([data2,data[target]], axis =1)
    # else:
    #     data1 =data2
    # data1 = data1.select_dtypes(exclude=['object'])
    # data1 = data1[data1[target].notna()] 
    
    data1 = data[data[target].notna()]
...
def recategorize(data):
    ...
    # data['GCOW'].replace({2:0},inplace = True)
    # data['GCAT'].replace({1:0,3:1},inplace = True)
    # data['GDOG'].replace({3:0},inplace = True)
