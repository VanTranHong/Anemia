...
def bag_xgboost(X_train,X_test,y_train,y_test):
    ...
    df = pd.DataFrame(columns=['Max_depth','N_estimators','Estimators','Confusion Matrix'])
    rows = []
    therate = 0.05
    ...
    for depth in max_depth:
        for ests in n_estimators:
            estimator = XGBClassifier(booster='gbtree',max_depth=depth,learning_rate=therate,n_estimators = ests)
            for n_est in estimators:
                ...
                tn, fp, fn, tp = confusion_matrix(y_test, predicted_labels, labels=[0,1]).ravel()
                convert_matrix = [tn,fp,fn,tp]
                rows.append([depth,estimators,n_est,  convert_matrix])

    for i in range(len(rows)):
        df = df.append({'Max_depth':rows[i][0],'N_estimators':rows[i][1],'Estimators':rows[i][2],
                        'Confusion Matrix':rows[i][3]}, ignore_index=True)
